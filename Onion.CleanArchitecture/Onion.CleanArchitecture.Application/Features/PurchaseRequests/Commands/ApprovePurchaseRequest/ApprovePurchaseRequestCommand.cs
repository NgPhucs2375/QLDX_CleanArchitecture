...
namespace Onion.CleanArchitecture.Application.Features.PurchaseRequests.Commands.ApprovePurchaseRequest
{
    ...
    public class ApprovePurchaseRequestCommandHandler : IRequestHandler<ApprovePurchaseRequestCommand, Response<int>>
    {
        ...
        public ApprovePurchaseRequestCommandHandler(IPurchaseRequestRepositoryAsync repository, IApprovalRecordService approvalRecordService)
        {
            _repository = repository;
            _approvalRecordService = approvalRecordService;
        }

        public async Task<Response<int>> Handle(ApprovePurchaseRequestCommand request, CancellationToken ct)
        {
            ...
            if (!machine.CanFire(PurchaseRequestTrigger.Approve))
                throw new ApiException("Cannot approve in current status");

            machine.Fire(PurchaseRequestTrigger.Approve);
            await _repository.UpdateAsync(entity);
            await _approvalRecordService.RecordAsync(entity, PurchaseRequestTrigger.Approve, string.Empty, ct);
