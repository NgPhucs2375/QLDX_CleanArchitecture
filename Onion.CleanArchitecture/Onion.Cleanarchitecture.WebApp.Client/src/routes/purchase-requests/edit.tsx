...
import { Form, Input, Select, Typography, Row, Col, Button, Card, Space, Tag, Divider } from "antd";
...
export const EditPurchaseRequest = () => {
  const form = usePurchaseRequestForm("edit");
  const cascade = useCascadeData();
  const { queryResult } = form.form;
  const { formProps, saveButtonProps } = form.form;
  ...
  useEffect(() => {
    form.initEditData(initialData, cascade.loadCascade, cascade.loadProducts);
  }, [initialData]);

  useEffect(() => {
    if (cascade.cascadeData.categories.length > 0) {
  ...
  return (
    <Edit
      ...
      footerButtons={() => (
        <Button type="primary" size="large" onClick={() => form.handleSubmit()}
            ...
            <Row gutter={24}>
              ...
              {cascade.cascadeData.approvers.filter((a) => a.role !== "Trưởng đơn vị").map((a, idx) => (
                <Col xs={24} md={8} key={idx}>
                  ...
                  <Tag color="geekblue" style={{ padding: "4px 12px", fontSize: 14 }}>{a.approverName || a.approverId}</Tag>
                </Col>
              ))}
              <Col xs={24} md={8}>
                ...
                <Tag color="green" style={{ padding: "4px 12px", fontSize: 14 }}>
                  {(initialData?.Approvers ?? []).find((a) => a.StepOrder === 3)?.ApproverName ?? initialData?.CreatedBy ?? "Đang tải..."}
                </Tag>
              </Col>
            </Row>
          </Card>
        ...
        {cascadeAvailable && (
          ...
          >
            ...
            <Space direction="vertical" size="large" style={{ width: "100%" }}>
              {form.selectedCategories.map((cat, index) => {
                ...
                return (
                  <Card key={`${cat.categoryId}-${index}`} className="pr-card" size="small"
                    ...
                    extra={<Button danger type="text" icon={<DeleteOutlined />} onClick={() => form.removeCategory(cat.categoryId)} />}
                  >
                    <ProductTable
                      products={prods}
                      items={cat.items}
