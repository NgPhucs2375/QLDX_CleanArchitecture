...
export const ShowCategory = () => {
  const { queryResult: { isLoading, data } } = useShow<ICategory>();
  const record = data?.data;

  return (
    <Show 
      isLoading={isLoading}
      title={<Title level={3} style={{ margin: 0, color: '#476481', fontWeight: 700 }}>Chi Tiết Danh Mục</Title>}
    >
      <Card bordered={false} style={{ background: '#ffffff', border: '1px solid #e1e7ee', borderRadius: 8, boxShadow: '0 2px 10px rgba(122, 157, 193, 0.05)' }}>
        ...
        <Row gutter={[32, 24]} style={{ padding: '0 8px' }}>
          ...
          <Col xs={24} sm={12} lg={6}>
            <Title level={5} style={{ marginTop: 0, color: '#6b7c93', fontSize: 14 }}>Mã Danh Mục</Title>
            <Text strong style={{ fontSize: 16, color: '#476481' }}>{record?.Code}</Text>
          </Col>
          <Col xs={24} sm={12} lg={6}>
            <Title level={5} style={{ marginTop: 0, color: '#6b7c93', fontSize: 14 }}>Tên Danh Mục</Title>
          ...
          <Col xs={24} sm={12} lg={6}>
            <Title level={5} style={{ marginTop: 0, color: '#6b7c93', fontSize: 14 }}>Người Cập Nhật</Title>
            <Text>{record?.LastModifiedBy || '—'}</Text>
          </Col>
          <Col xs={24} sm={12} lg={6}>
