...
{
    public class NotifyControlApprovedConsumer : IConsumer<NotifyControlApprovedCommand>
    {
        private readonly IHubContext<NotificationHub> _hub;
        private readonly INotificationStore _noti;
        private readonly ILogger<NotifyControlApprovedConsumer> _logger;

        public NotifyControlApprovedConsumer(
            IHubContext<NotificationHub> hub,
            INotificationStore noti,
            ILogger<NotifyControlApprovedConsumer> logger)
        {
            _hub = hub;
            _noti = noti;
            _logger = logger;
        ...
        public async Task Consume(ConsumeContext<NotifyControlApprovedCommand> context)
        {
            var cmd = context.Message;
            var noti = new Notification
            {
                UserId = cmd.UserId,
                Title = "Phiếu đề xuất đã được kiểm soát duyệt",
                Body = cmd.Message,
